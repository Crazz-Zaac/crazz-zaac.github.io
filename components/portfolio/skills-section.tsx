  ...
  return (
    <div className="flex flex-col gap-6 mt-6 mb-2">
      {SKILL_GROUPS.map(({ category, items }) => (
        <div key={category}>
          <h4 className="text-xs font-semibold tracking-normal text-muted-foreground uppercase mb-3">
            {category}
          </h4>
          <div className="flex flex-wrap gap-2">
            {items.map((item) => (
              <span
                key={item}
                className="inline-flex items-center gap-1.5 rounded-md border border-border bg-secondary/50 px-2.5 py-1 text-xs text-secondary-foreground transition-colors duration-200 hover:border-primary/50 hover:text-primary"
              >
